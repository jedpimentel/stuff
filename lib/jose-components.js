...
AFRAME.registerComponent("joses-default-interface", {
  init: function() {
    ...
    const headset_connected = AFRAME.utils.device.checkHeadsetConnected()
    console.log('headset_connected', headset_connected)
    // weak test, but enough for between desktop/quest
    const has_positional = AFRAME.utils.device.checkHasPositionalTracking()


    // https://stackoverflow.com/questions/46219330/how-do-i-check-if-user-is-using-a-headset-instead-of-mouse
    ...
    this.el.innerHTML =  `
      <a-entity id="camera-wrapper">
        ...
        <a-entity pos-trail-line laser-controls="hand: left" grip-to-move="targetID: camera-wrapper" raycaster="objects:.clickable; far:4">
          ...
          <a-box  height="0.02" width="0.05" depth="0.05" position="0 -0.05 0" color="red"></a-box>
        </a-entity>
