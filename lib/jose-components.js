...
/*
      ...
      ╚═════╝ ╚══════╝╚═╝     ╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝   
       ...
       - Targets:
                  - Oculus Quest
                  - Mouse & Keyboard
       - NEED: object dispenser
*/

// move into its own section????
AFRAME.registerComponent('raycaster-debugger', {
  init: function () {
    ...
    voxel.setAttribute('position', '0 0.1 -1')
...
AFRAME.registerComponent("joses-default-interface", {
  schema: {
    mode: {type: 'string'},// "wasd" to force MOUSE_AND_KEYBOARD_UI
  },
  init: function() {
    ...
    // 0.8.2-migration: checkHasPositionalTracking was removed on 1.0.1
    const has_positional = AFRAME.utils.device.checkHasPositionalTracking()
    // let has_positional;//bool rep if has tracking;
    // if (AFRAME.utils.device.checkHasPositionalTracking) {
    //   // assume 0.8.2 and handle it for now
    //   has_positional = AFRAME.utils.device.checkHasPositionalTracking()
    // } else {
    //   // I think this existed on 0.8.2, think it wasn't working for me
    //   // works on 1.0.1, but that ver breaks other stuff
    //   has_positional = AFRAME.utils.device.checkHeadsetConnected()
    // }
    ...
    const MOUSE_AND_KEYBOARD_UI = `
      <a-entity id="camera-wrapper">
        <a-camera look-controls="pointerLockEnabled: true" near="0.001" wasd-controls>
            <a-cylinder id="fing-longer" position="0.2 -0.4 -0.9" radius="0.01" height= "1.0" rotation="90 0 0" color="beige" opacity="1.0"></a-cylinder>
            <a-entity pos-trail-line="segments: 32" position="0.2 -0.4 -2"></a-entity>
            <a-entity position="0.2 -0.4 -1.4" cursor="rayOrigin: entity; showLine: true;" raycaster="objects: .clickable; showLine: true; far: 16" line="color: white; opacity:0.5"></a-entity>
        </a-camera>
        <a-box id="home-button" class="clickable" height="0.02" width="0.05" depth="0.05" position="0 -0.05 0" color="red"></a-box>
      </a-entity>
    `
    ...
    // not needed anymore?
    // this.el.innerHTML = this.data.mode === 'wasd' ? MOUSE_AND_KEYBOARD_UI : POSITIONAL_UI;

    if (window.hasNativeWebVRImplementation) {
      this.el.innerHTML = POSITIONAL_UI;
    } else {
      this.el.innerHTML = MOUSE_AND_KEYBOARD_UI;
    }
...
// raycaster that draws lines on the surfaces it intersects
// z-level/positional stuff could be an issue on curved surfaces
// drawing on inside of curve should work better that on outside

// A)
//    for now is a sphere, that casts a ray towards -Z, and auto-draws at intersection points
// B)
//    maybe better as somethng that can be socketed to a raycaster?
//    

// Maybe have lines for traces and particles for non-continuous intersections
// AFRAME.registerComponent('a-crayon', {
// this is being on on the raycaster, but could also be implemented as a raytarget component
AFRAME.registerComponent('draw-on-intersection', {
  dependencies: ['raycaster'],
  schema: {
    // targetID: {type: 'string', default: 'n/a'}
    segments: {type: 'number', default: 72*60*90}// 1.5 hours @ 72 fps
    // segments: {type: 'number', default: 72*4}//Oculus Quest target framerate: 72
  },
  init: function() {
    this.segment_index = 0;
    this.trace_position = new THREE.Vector3();
    this.last_pos = new THREE.Vector3();
    this.active = false;
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.addEventListener('raycaster-intersection', e => this.active = this.new_trace = true);
      document.addEventListener('raycaster-intersection-cleared', e => this.active = false);

    })
    
  },
  tick: function() {
    if(this.active && this.raycaster.intersections[0]) {//dirty

      // const intersection = this.raycaster.getIntersection(this.el)
      const point = this.raycaster.intersections[0].point
      // console.log(point)

      // WARNING !!! this component needs major cleaning

      if(this.new_trace) {
        // used to avoid connected new traces with old trace
        // this.el.object3D.getWorldPosition(this.trace_position);
        this.trace_position.x = point.x;
        this.trace_position.y = point.y;
        this.trace_position.z = point.z;
        // console.log(this.trace_position)
        this.new_trace = false;
      } else {
        if(this.segment_index === this.data.segments) { this.segment_index = 0; };
        this.last_pos.copy(this.trace_position)
        // this.el.object3D.getWorldPosition(this.trace_position);
        this.trace_position.x = point.x;
        this.trace_position.y = point.y;
        this.trace_position.z = point.z;
        ...
        this.geometry.vertices[this.segment_index * 2    ].copy(this.last_pos);
        this.geometry.vertices[this.segment_index * 2 + 1].copy(this.trace_position);
        this.geometry.computeBoundingSphere();
        this.geometry.verticesNeedUpdate = true;
        this.segment_index++;
        // console.log('a')
