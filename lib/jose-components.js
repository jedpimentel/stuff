...
class Robot {
  ...
  rumFromParsedFiles(name) {
    ...
    // first use GLTFExporter to get object3D data
    // then input it as file data
    let file = new File(this.parsedFiles, `${name}.rum"`, {
  ...
  imgFileToEntity(file) {
    ...
    if(file.type.includes('image/')) {
      ...
      reader.onload = (e) => {
        ...
        console.log('reader stuff', reader)
      ...
      // return `<a-image src=${imgURL} position="0 1 -2"></a-image>`
    }
  };
  fromBase64() {
